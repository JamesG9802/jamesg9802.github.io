...
export class Simulation {
    ...
    static async create(canvas: HTMLCanvasElement): Promise<Simulation | undefined> {
        ...
        if(engine != undefined)
        {
            ...
            for(let i = 0; i < 1000; i++) {
                let model = await Model.load_from_file(engine.device, engine.model_pipeline, Math.random() > 5 ? "cube" : "cliff");
                if(!model) continue;
                let angle = Math.random() * 2 * Math.PI;
                let range = Math.random() * 26 + 4;
    ...
    resize(width: number, height: number) {
        ...
        this.engine.resize();
        this.world.main_camera.project_matrix = Camera.compute_projection_matrix(
            Math.PI / 5, this.aspect_ratio, .1, 100
        );
    }
